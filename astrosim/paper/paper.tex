...
A \textbf{vigesimal (base-20) sub-module} implements the Maya positional
numeration system \citep{ascher2002} to express astronomical distances and
orbital periods. Every distance is converted to integer kilometres and
represented in base-20 using the Maya dot-and-bar glyph notation
($\bullet = 1$, $\overline{\phantom{x}} = 5$, $\emptyset = 0$), with
Yucatec Maya digit names and positional place names (\textit{kin},
\textit{winal}, \textit{tun}, \textit{katun}, \textit{baktun}).
Orbital periods are expressed in the Maya Long Count calendar, connecting
the mathematical structure used by Maya astronomers to model Venus cycles
(584~days) and the solar year (365~days) with the same quantities computed
by the Keplerian engine. This module demonstrates that numerical base is a
cultural construct rather than a physical necessity, providing an
ethno-mathematical entry point for learners from Mesoamerican heritage
communities.
